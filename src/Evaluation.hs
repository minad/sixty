...
apply fun plicity arg =
  case fun of
    ...
    Domain.Neutral hd spine ->
      pure $ Domain.Neutral hd $ spine Domain.:> Domain.Apps (pure (plicity, arg))

    Domain.Glued hd spine value -> do
      appliedValue <- lazy $ do
        value' <- force value
        apply value' plicity arg
      pure $ Domain.Glued hd (spine Domain.:> Domain.Apps (pure (plicity, arg))) appliedValue
...
applyElimination value elimination =
  case elimination of
    Domain.Apps args ->
      foldlM (\v -> uncurry $ apply v) value args

    Domain.Case branches ->
      case_ value branches

applySpine :: Domain.Value -> Domain.Spine -> M Domain.Value
applySpine value (Domain.Spine elims) =
  foldlM applyElimination value elims

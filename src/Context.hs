...
dependencies context value = do
  ...
  case value' of
    Domain.Neutral hd (Domain.Spine elims) -> do
      hdVars <- headVars hd
      elimVars <- mapM eliminationDependencies elims
    ...
    Domain.Glued (Domain.Global _) (Domain.Spine elims) _ ->
      fold <$> mapM eliminationDependencies elims
...
newMeta type_ context = do
  ...
  pure $ Domain.Neutral (Domain.Meta i) $ Domain.Empty Domain.:> Domain.Apps ((,) Explicit . Domain.var <$> IntSeq.toTsil (boundVars context))
